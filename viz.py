...
99.990%         0.024 sec."""
    ...
    benchmark_text = st.text_area("Paste your benchmark results here:", 
                                value=default_data, 
                                height=300)
    ...
    if benchmark_text:
        ...
        with col3:
            st.metric("Median (P50) Latency", f"{df[df['percentile'] == 50.0]['latency'].iloc[0]:.3f} sec")
        
        with col4:
            st.metric("P95 Latency", f"{df[df['percentile'] == 95.0]['latency'].iloc[0]:.3f} sec")
        
        with col5:
            st.metric("P99 Latency", f"{df[df['percentile'] == 99.0]['latency'].iloc[0]:.3f} sec")
